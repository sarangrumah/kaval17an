...
const WARNA = [
  "bg-red-700", "bg-red-600", "bg-red-500", "bg-amber-500",
    ...
    <>
      ...
      <main className="mx-auto max-w-4xl px-3 py-6 sm:px-6">
        <header className="mb-5">
          <Eyebrow className="text-red-700">Panitia HUT RI ke-81 &middot; Kavling AL</Eyebrow>
        ...
        {/* Papan kas */}
        <section className="rounded-lg bg-slate-900 p-5 text-stone-50 shadow-lg sm:p-7">
          ...
          <div className="mt-5 flex h-4 w-full overflow-hidden rounded-full">
            {alokasi.map((p, i) => (
              <div key={p.id} title={`${p.nama} · ${rp(p.bagian)}`}
                   ...
                   className={WARNA[i % WARNA.length]} />
            ))}
          </div>
